class GameRenderer {
        ...
        for (const [char, path] of Object.entries(tilePaths)) {
            try {
                ...
                await new Promise((resolve, reject) => {
                    img.onload = resolve;
                    img.onerror = () => reject(`Failed to load image for ${char} from ${path}`);
                });
    ...
    render() {
        ...
        // First render the default 'A' tile layer
        const defaultTile = this.tileImages['A'];
        if (defaultTile) {
            ...
            console.warn("Default tile 'A' not found in tileImages");
        }
        
        // Then render the game map on top
        for (let y = 0; y < map.length; y++) {
            for (let x = 0; x < map[y].length; x++) {
                const tile = map[y][x];
                if (tile === 'A') continue; // Skip 'A' tiles since we already drew them
                ...
                if (img) {
                    ...
                    console.warn(`No image for tile ${tile} at ${x},${y}`);
                }
            }
        }
    }
